 ...
 */
define(['app', 'lodash', 'Debug', 'js/eventBus', 'js/configurationService'], function (app, _, Debug, eventBus) {
  'use strict';
  /**
  ...
  exports.setAutoTestMode = function () {
    _autoTestMode = true;
  };
  /**
  ...
  exports.enable = function (useInternalServer) {
    ...
    var $q = injector.get('$q'); // I'm calling requirejs twice to allow for bundling of the analytics into the piwik file. If we request
    // both from requirejs at the same time, it forces 2 files to be pulled from the server.

    return $q(function (resolve) {
      requirejs(['lib/piwik/analytics'], function () {
        if (_authenticated) {
          var cfgSvc = injector.get('configurationService');
          var solution;
          $q.all([cfgSvc.getCfg('OOTB_Verification').then(function (OOTB_Verification) {
            ...
            cfgSvc.getCfg('versionConstants').then(function (versionConstants) {
              ...
              San.initialize(solution.solutionId, versionConstants.version, useInternalServer || undefined, app.getBaseUrlPath() + '/lib/piwik/piwik.js');
              _enabled = true;
  ...
  exports.disable = function (useInternalServer, encVendId) {
    ...
    var injector = app.getInjector(); // requirejs is called twice to allow for bundling of the analytics into the piwik file.
    // Requesting both from requirejs at the same time pulls 2 files from the server.
    ...
    requirejs(['lib/piwik/analytics'], function () {
      if (_authenticated) {
        ...
        cfgSvc.getCfg('solutionDef').then(function (solutionDef) {
          ...
          // Initialize San with solution id & client version
          San.initialize(solution.solutionId, versionConstants.version, useInternalServer || undefined, app.getBaseUrlPath() + '/lib/piwik/piwik.js'); // If Analytics was previously enabled, we will log one "participating=false" event.
  ...
  exports.logPageViewEvent = function () {
    var injector = app.getInjector();
  ...
  function _getResolutionZoomInfo() {
    ...
    if (resolutionZoomData) {
      ...
      if (!(resolutionZoomData.width === new_resolutionZoomData.width && resolutionZoomData.height === new_resolutionZoomData.height && resolutionZoomData.zoom === new_resolutionZoomData.zoom)) {
        ...
        exports.logCommands(JSON.stringify(new_resolutionZoomData));
      }
    } else {

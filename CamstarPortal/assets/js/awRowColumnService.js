 ...
 */
define(['app', 'angular', 'lodash', 'js/logger'], function (app, ngModule, _, logger) {
  'use strict';
  /**
  ...
  app.service('awRowColumnService', [function () {
    ...
    self.initRowOrColumn = function (scopeElements) {
      ...
      containerElement.style.display = 'flex'; // containerElement.style.flexWrap = 'wrap';
      ...
      var fillData = self.getFillData(areaList);
      ...
      _.forEach(areaList, function (area, index) {
        ...
        self.setAreaStyle(area.areaElement, area.sizeType, area.sizeValue, area.color, fillAreaString, area.alignContent, area.when, area.areaType); // Capture the index of the first center and (right or bottom) elements
        ...
        if (offsetValue > 0) {
          self.addNewElement(containerClassName, area.offsetSizeType, offsetValue, 'transparent', '', insertBefore, area.areaElement, area.areaType);
        }
      ...
      if (fillValue > 0) {
        ...
        if (centerJustifyIndex >= 0) {
          ...
          self.addNewElement(containerClassName, 'P', 0, 'transparent', fillAreaString, true, areaList[centerJustifyIndex].areaElement, areaList[0].areaType);

          if (farJustifyIndex > 0) {
            // Also insert a new element before the far justify element
            self.addNewElement(containerClassName, 'P', 0, 'transparent', fillAreaString, true, areaList[farJustifyIndex].areaElement, areaList[0].areaType);
          } else {
            // There is no right or bottom justify option - so insert at the end to center
            self.addNewElement(containerClassName, 'P', 0, 'transparent', fillAreaString, false, scopeElements[0], areaList[0].areaType);
          }
    ...
    self.addNewElement = function (className, sizeType, sizeValue, color, fillAreaString, before, sibling, areaType) {
      var newElements = ngModule.element('<div' + ' class=\'' + className + '\'></div>');
      self.setAreaStyle(newElements[0], sizeType, sizeValue, color, fillAreaString, null, null, areaType); // Insert the new element after the given element

      var currentScopeElement = ngModule.element(sibling);
      currentScopeElement.after(newElements);

      if (before) {
        // Note there is no direct access to the JQuery "before" method
        // So this will switch places to achieve an insert "before"
        newElements.after(currentScopeElement);
      }
    };
    /**
     * Set Area Style
    ...
    self.setAreaStyle = function (areaElement, sizeType, sizeValue, color, fillAreaString, alignContent, when, areaType) {
    ...
    self.isLastChild = function (parent, child) {
      if (!parent) {
        return false;
      }
    ...
    self.getFillData = function (areaList) {
      ...
      if (typeError) {
        self.reportError('area found that is defined with mixed rows and columns');
      }

      if (fillSize < 0) {
        fillSize = 0;
        self.reportError('row or column found defined with proportional areas that exceed grid system size: ' + self.constants.gridSystemSize);
      }
    ...
    self.getAreaList = function (containerElement) {
      if (!containerElement) {
        return null;
      }
      ...
      _.forEach(childList, function (subArea) {
        var subAreaData = self.getAreaData(subArea);
    ...
    self.getAreaData = function (areaElement) {
      ...
      if (classList.contains('aw-layout-row')) {
        ...
        standardSizeFactor = self.constants.standardHeightFactor;
      } else if (classList.contains('aw-layout-column')) {
        areaType = 'column';
        stackDirection = 'row'; // flexbox direction

        standardSizeFactor = self.constants.standardWidthFactor;
      } else {
        // Ignore all other elements
        return null;
      }

      var userAttributes = self.getAreaUserAttributes(areaElement);
      var sizeData = self.getSizeDataFromAttribute(userAttributes.size, standardSizeFactor);
      var sizeType = sizeData.sizeType;
      var sizeValue = sizeData.sizeValue;
      sizeData = self.getSizeDataFromAttribute(userAttributes.offset, standardSizeFactor);
    ...
    self.getAreaUserAttributes = function (areaElement) {
    ...
    self.getSizeDataFromAttribute = function (sizeAttr, fixedSizeFactor) {
      ...
      if (isNaN(sizeValue)) {
        self.reportError('invalid row, column or offset size value (' + sizeAttr + ') - using default of 1');
        sizeValue = 1;

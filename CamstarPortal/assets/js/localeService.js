 ...
 */
define(['app', 'lodash', 'js/logger', 'js/eventBus', 'js/browserUtils', 'js/configurationService', 'js/localStorage', 'config/installedLocales'], function (app, _, logger, eventBus, browserUtils, cfgSvc, localStrg) {
  'use strict';
  /**
  ...
  function resolveLocale() {
    ...
    if (localeFromURL) {
      ...
      if (!localeCode) {
        ...
        logger.error('The language resource file <myComponent>Messages_<' + localeCode + '>.json is not present. Removing from URL.');
    ...
    if (!localeCode) {
       ...
       */
      logger.error('The language resource file <myComponent>Messages_<' + localeCode + '>.json is not present. Changing default locale to en_US.');
  ...
  app.factory('localeService', ['configurationService', function (cfgSvcNg) {
    _cfgSvc = cfgSvcNg;

 ...
 */
define(['app', 'lodash', 'assert', 'Debug', 'js/logger', 'js/eventBus', 'js/browserUtils', 'js/localStorage', 'soa/kernel/clientDataModel', 'soa/kernel/clientMetaModel', 'soa/kernel/propertyPolicyService', 'js/appCtxService', 'soa/kernel/typeCacheService', 'js/configurationService'], function (app, _, assert, Debug, logger, eventBus, browserUtils, localStrg) {
  'use strict';
  ...
  function isMap(state, schemaElement) {
    if (Object.keys(schemaElement).length === 2) {
      if (schemaElement.hasOwnProperty('key') && schemaElement.hasOwnProperty('value') && !state.schemaService.hasOwnProperty(schemaElement.key)) {
        return true;
      }
    }
    ...
    switch (typeName) {
      ...
      case 'Date':
        if (!_.isString(bodyElement)) {
          state.issues.push('INVALID FIELD: Expected string, not ' + _typeof(bodyElement) + ' --' + state.stack.join('.'));
        }
      ...
      case 'double':
        if (!_.isNumber(bodyElement)) {
          state.issues.push('INVALID FIELD: Expected number, not ' + _typeof(bodyElement) + ' --' + state.stack.join('.'));
        }
      ...
      case 'boolean':
        if (!_.isBoolean(bodyElement)) {
          state.issues.push('INVALID FIELD: Expected boolean, not ' + _typeof(bodyElement) + ' --' + state.stack.join('.'));
        }
      ...
      default:
        ...
        if (state.schemaService.hasOwnProperty(typeName)) {
          ...
          if (!_.isArray(bodyElement)) {
            state.issues.push('INVALID FIELD: Expected array, not ' + _typeof(bodyElement) + ' --' + state.stack.join('.'));
            return;
          }
          ...
          _.forEach(bodyElement, function (value, key) {
            ...
            if (!_.isString(key)) {
              state.issues.push('INVALID FIELD: Expected string, not ' + _typeof(key) + ' --' + state.stack.join('.'));
              return;
            }
  ...
  function processResponseObjects(response, modelObjs) {
    ...
    if (modelObjs && modelObjs.length > 0) {
      // Add objects to CDM
      _cdm.cacheObjects(modelObjs); // To support the anti-pattern of code pulling the modelObject from the response, we need to update the response serviceData.
  ...
  exports.postUnchecked = function (serviceName, operationName, body, propertyPolicyOverride, ignoreHost) {
    ...
    return promise.then(function (schemaService) {
      ...
      return $http.post(browserUtils.getBaseURL() + 'tc/JsonRestServices/' + endPt, jsonData, {
        ...
        assert(response, 'No response given for ' + endPt); // AW-66303 - report Teamcenter Web App Server info.
        ...
        assert(typeof body !== 'string' || body.indexOf('<?xml version') === -1, 'Unexpected response body for: ' + endPt);
        return body;
      });
    }).then(function (response) {
      ...
      if (serviceData) {
        ...
        if (responseObjects.length > 0) {
          ...
          if (_propPolicySvc.checkForSelectedObject(null, responseObjects)) {
            ...
            exports.postUnchecked('Core-2006-03-DataManagement', 'getProperties', input, propertyPolicyOverride);
          }
        }
      }
    }
  ...
  exports.Operation = function (serviceName, operationName, body, propertyPolicyOverride) {
    ...
    this.executeUnchecked = function () {
      return exports.postUnchecked(this._serviceName, this._operationName, this._body, this._propertyPolicyOverride);
    };
  ...
  exports.getTCSessionInfo = function () {
    ...
    return exports.post(GET_SESSION_INFO.serviceName, GET_SESSION_INFO.operationName).then(function (response) {
      ...
      if (err.cause && err.cause.status === 500) {
        // "Internal Server Error" which typically means that this API isn't availabe on the server
        _$state.go('serverError');
      }

      _propPolicySvc.unregister(policyId); // Since we have no session, clear the session from localStorage to ensure we don't try to use it.
    ...
    if (missingTypeNames.length > 0) {
      ...
      return getTypeDescriptions(missingTypeNames).then(function (responseGetTypeDescriptions) {
        if (responseGetTypeDescriptions && responseGetTypeDescriptions.types) {
          ...
          _cmm.cacheTypes(modelTypes); // Cache the types in localStorage.
  ...
  app.factory('soa_kernel_soaService', ['$q', '$http', '$injector', '$state', 'soa_kernel_clientDataModel', 'soa_kernel_clientMetaModel', 'soa_kernel_propertyPolicyService', 'appCtxService', 'typeCacheService', 'configurationService', function ($q, $http, $injector, $state, cdm, cmm, propPolicySvc, appCtxSvc, typeCacheSvc, configSvc) {
    _$q = $q;
    _$http = $http;
    _$injector = $injector;
    _$state = $state;

 ...
 */
define(['app', 'lodash', 'js/eventBus', 'soa/kernel/clientMetaModel', 'js/dateTimeService', 'js/localeService', 'js/sanitizer'], function (app, _, eventBus) {
  'use strict';
  /**
    ...
    switch (propertyDescriptor.valueType) {
      ...
      case 8:
        ...
        if (propertyDescriptor.name !== 'fnd0SVG') {
          ...
          propToUpdate.dbValues = _sanitizer.sanitizeHtmlValues(propToUpdate.dbValues); // Sanitize the String UI values
  ...
  var _convertUTCTimeValues = function _convertUTCTimeValues(values) {
    if (!_dateTimeSvc) {
  ...
  var _updateProps = function _updateProps(props, modelType) {
    if (!_dateTimeSvc) {
      _dateTimeSvc = app.getInjector().get('dateTimeService');
  ...
  var setUserSession = function setUserSession(userSession) {
  ...
  app.factory('soa_kernel_clientDataModel', ['soa_kernel_clientMetaModel', 'dateTimeService', 'sanitizer', function (cmm, dateTimeSvc, sanitizer) {
    _cmm = cmm;

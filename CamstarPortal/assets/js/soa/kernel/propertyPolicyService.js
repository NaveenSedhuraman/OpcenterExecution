 ...
 */
define(['app', 'lodash', 'js/eventBus', 'js/logger', 'js/dateTimeService', 'soa/kernel/clientMetaModel'], function (app, _, eventBus, logger) {
  'use strict';
  /**
   * Angular services
   */
  ...
  exports.unregister = function (policyId) {
    ...
    if (exports._policyId2policy.hasOwnProperty(policyId)) {
      ...
      if (logger.isTraceEnabled()) {
        logger.trace('Unregister Property Policy Id: ' + policyId + ' TimeStamp: ' + _dateTimeSvc.formatTime(Date.now()));
      }
        ...
        _.forEach(policy.types, function (type) {
          ...
          if (existingType) {
            if (type.properties) {
              _.forEach(type.properties, function (property) {
                if (property) {
                  var existingProp = _.find(existingType.properties, _.matchesProperty('name', property.name));

                  if (existingProp) {
                    if (property.modifiers) {
                      _.forEach(property.modifiers, function (modifier) {
                        _.remove(existingProp.modifiers, _.matchesProperty('name', modifier.name));
                      });
                    } else {
                      _.remove(existingType.properties, _.matchesProperty('name', existingProp.name));
                    }
                  }
                }
              });
  ...
  app.factory('soa_kernel_propertyPolicyService', ['$q', '$http', 'dateTimeService', 'soa_kernel_clientMetaModel', function ($q, $http, dateTimeSvc, cmm) {
    _$http = $http;
    _$q = $q;

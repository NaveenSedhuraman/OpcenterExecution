 ...
 */
define(['app', 'js/eventBus', 'soa/kernel/soaService', 'soa/kernel/clientDataModel', 'js/appCtxService'], function (app, eventBus) {
  'use strict';
  ...
  exports.signOut = function () {
    return _soaSvc.postUnchecked('Core-2006-03-Session', 'logout', {}, {}).then(function (response) {
      _soaSvc.setSessionInfo(true); // both SSO and UserPw call this, so fire the event here.
  ...
  exports.setUserSessionState = function (pairs) {
    return _soaSvc.post('Core-2015-10-Session', 'setUserSessionStateAndUpdateDefaults', {
      ...
      _soaSvc.setSessionInfo(); // With a successful change in session, we force a reload for security concerns & memory leaks.
  ...
  exports.setUserSessionStateWithoutDefaults = function (pairs) {
    return _soaSvc.post('Core-2007-12-Session', 'setUserSessionState', {
      pairs: pairs
    }).then(function (response) {
      _soaSvc.setSessionInfo(); // With a successful change in session, we force a reload for security concerns & memory leaks.
  ...
  app.factory('soa_sessionService', ['soa_kernel_soaService', 'soa_kernel_clientDataModel', 'appCtxService', function (soaSvc, cdm, appCtxSvc) {
    _soaSvc = soaSvc;
    /**

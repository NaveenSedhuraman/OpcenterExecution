...
{
    public class dexCombineQty : MatrixWebPart
    {
        ...
        public void LoadContainersGrid()
        {
            ...
            if (manualContainer == null || (manualContainer != null && manualContainer.IsEmpty))
                ...
                Page.Service.ExecuteFunction("dexCombine", "Load");
        ...
        public void CombineContainerControl_DataChanged(object sender, EventArgs e)
        {
            ...
            var data = new OM.dexCombine()
                                {
                                    EligibleContainersInquiry = new OM.dexEligibleContainersInquiry
                                                                {
            ...
            var request = new dexCombine_Request()
                            {
                                Info = new dexCombine_Info()
                                        {
                                            EligibleContainersInquiry = new OM.dexEligibleContainersInquiry_Info { EligibleContainer = FieldInfoUtil.RequestSelectionValue() }
                                        }
                            };

            var fs = FrameworkManagerUtil.GetFrameworkSession();
            var service = new dexCombineService(fs.CurrentUserProfile);
            dexCombine_Result result;
        ...
        public override void GetInputData(Service serviceData)
        {
            ...
            if (EligibleContainerGrid.Data != null && EligibleContainerGrid.GridContext.SelectedRowIDs != null)
            {
                ...
                (serviceData as OM.dexCombine).FromContainerDetails = list.Select(n => new CombineFromDetail()
                                                                                  {
        ...
        public override void DisplayValues(Service serviceData)
        {
            ...
            if ((serviceData as OM.dexCombine).FromContainerDetails != null)
            {
                foreach (var detail in (serviceData as OM.dexCombine).FromContainerDetails)
        ...
        protected void EligibleContainerGrid_GetRowSnapItem(object item, IEnumerable<DataColumn> dataColumns, DataRow row)
        {
            ...
            {
                row["Qty"] = OriginalQty[(item as OM.CombineFromDetail).FromContainer.ToString()].ToString();

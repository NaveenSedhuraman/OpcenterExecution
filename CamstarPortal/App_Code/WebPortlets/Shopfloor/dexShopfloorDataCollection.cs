...
namespace Camstar.WebPortal.WebPortlets
{
    ...
    /// </summary>
    public class dexShopfloorDataCollection : MatrixWebPart
    {
        public dexShopfloorDataCollection()
        {
        ...
        protected virtual void _Container_DataChanged(object sender, EventArgs e)
        {
            ...
            //Added to auto populate data collection from operational view
            if (Page.DataContract.GetValueByName("SelectedDCFromAction") != null)
            {
                string sSelectedDC = Page.DataContract.GetValueByName("SelectedDCFromAction").ToString();
                RevisionedObjectRef revDataCollection = new RevisionedObjectRef(sSelectedDC.Split(':')[0], (sSelectedDC.Split(':')[1]).Trim());
                revDataCollection.RevisionOfRecord = false;
                CollectionDef.Data = revDataCollection;
                _ContainerChanged = false;
 ScriptManager.RegisterStartupScript(Page.Form, Page.Form.GetType(), "setPopupTitle", "setTimeout(function(){ window.parent.$('.floating-frame').find('.header-parent > .header > span').text('"+ sSelectedDC + "');},1000);", true);
            }
        }
        ...
        {
            get { return Page.FindIForm("dexParametricDataWP") as WebPartBase; }
        }
